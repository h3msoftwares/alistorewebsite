...
- `Product.quantity` is a free-standing signed `Int` (may be 0 or negative — e.g. oversold) with **no bearing on `isActive`**. Separate from per-variant `stockQuantity`.
- `Product.saleType` (`PERCENT` 0–100 / `AMOUNT`) + `saleValue` express an optional sale — set both or neither. Product reads return a computed `effectivePrice` (post-sale, clamped ≥ 0) and `onSale` (`src/lib/pricing.ts`).
